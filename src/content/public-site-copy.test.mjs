...
test('every locale has the same complete public copy and verification method',()=>{const shape=v=>Array.isArray(v)?v.map(shape):typeof v==='object'?Object.fromEntries(Object.entries(v).map(([k,x])=>[k,shape(x)])):(assert.ok(typeof v==='string'&&v.trim()),'text');for(const copy of [publicSiteCopy,enquiryCopy])for(const locale of locales)assert.deepEqual(shape(copy[locale]),shape(copy.en));for(const p of Object.values(publicSiteCopy)){assert.equal(p.evidence.length,5);assert.equal(p.questions.length,5);assert.ok(p.stage.includes('AFSA'));assert.ok(p.shareholderBody.includes('10%'));assert.ok(p.shareholderBody.includes('Tech Hub Limited'));assert.ok(p.evidence[2].body.includes('NDVI'));assert.ok(Buffer.byteLength(p.solanaBody)>200);}});
